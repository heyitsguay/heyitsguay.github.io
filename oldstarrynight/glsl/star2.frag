...
void main() {
    ...
    vec2 xyRaw = gl_FragCoord.xy * uScreenInverse;
    vec2 cRaw = vCenter.xy * uScreenInverse;
    vec2 xy = vec2(1. - xyRaw.x, xyRaw.y);
    vec2 c = 0.5 * (vCenter.xy + 1.); // vec2(1. - cRaw.x, cRaw.y);
    c = vec2(1. - c.x, 1. - c.y);
    float d = length((vPos.xy - vCenter.xy) * uScreenInverse);
    float m = .005 / d;

    vec3 camColor = texture2D(uVideo, xy).rgb;
    float camB = min(1., uVidBrightBoost * dot(camColor, avg));
    float shadeFactor = m * (uVidMaskStrength * camB + (1. - uVidMaskStrength));
